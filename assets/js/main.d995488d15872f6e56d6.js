...
const headerFixedClasses = ["fixed-top", "justify-content-center", "top-vr", "blur-bg", "pt-3", "pb-3"];

const menuCallback = (entries) => {
    if (mainMenu) {
        entries.forEach((entry) => {
            if (entry.isIntersecting) {
                mainMenu.classList.remove(...headerFixedClasses);
            } else {
                mainMenu.classList.add(...headerFixedClasses);
            }
        });
    }
};
...
const submenu = document.querySelectorAll(".dropdown-submenu a.dropdown-submenu-toggle");
if (submenu.length > 0) {
...
const nextItem = document.querySelector("#swiper-item-next");
const prevItem = document.querySelector("#swiper-item-prev");
